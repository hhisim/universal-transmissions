...
const T: Record<string, Record<string, string>> = {
  ...
  tr: {
    ...
    desc: "150 sayfa ksenolinguistik sanat, aşkın geometri ve hiperboyutsal iletimler.",
  ...
  ru: {
    ...
    desc: "150 страниц ксенолингвистического искусства, трансцендентной геометрии.",
...
export default function OracleV2Page() {
  ...
  const fetchTTS = useCallback(async (text: string): Promise<string> => {
    if (!voiceOn) return "";
    try {
      const res = await fetch("/api/oracle-v2/tts", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text, lang, voice: voiceGender === "m" ? "standard" : "hd" }) });
      if (!res.ok) return "";
  ...
  const send = useCallback(async (text?: string, forceMode?: string) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/oracle-v2", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: m, mode: useMode, lang, speed }) });
