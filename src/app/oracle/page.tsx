    ...
    <>
          ...
          >
            <div className="oracle-oracle-column">
              <div className="oracle-inline-controls">
                <div className="oracle-inline-block">
                  <div className="oracle-shell-kicker">Transmission</div>
                ...
                <div className="oracle-inline-block">
                  <div className="oracle-shell-kicker">Engine</div>
                  <div className="oracle-rail-toggle-row">
                    <button onClick={() => setSpeed("fast")} data-active={speed === "fast"}>Fast</button>
                    <button onClick={() => setSpeed("deep")} data-active={speed === "deep"}>Deep</button>
                    {(["en", "tr", "ru"] as const).map(l => (
                      <button key={l} onClick={() => setLang(l)} data-active={lang === l}>{l.toUpperCase()}</button>
                    ))}
                    <button onClick={() => { setVoiceOn(!voiceOn); if (voiceOn) window.speechSynthesis?.cancel(); }} data-active={voiceOn}>{voiceOn ? "Voice On" : "Voice Off"}</button>
                  </div>
                </div>
                <div className="oracle-inline-block oracle-inline-seeds">
                  <div className="oracle-shell-kicker">Question Seeds</div>
                  <div className="oracle-inline-seed-row">
                    {starters.map((q) => (
                      <button key={q} onClick={() => send(q)} disabled={atLimit} className="oracle-seed-button">{q}</button>
                    ))}
                  </div>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-hidden oracle-chat-glass-card" style={{
              ...
              <div className="oracle-chat-card-head">
                <div>
                  <div className="oracle-shell-kicker">Transmission Well</div>
              ...
              <div ref={chatRef} className="oracle-scroll" style={{ minHeight: 300, maxHeight: "min(500px, 48vh)", overflowY: "auto", padding: "22px 24px 12px" }}>
                {!hasMessages && (
      ...
      <style>{`
        ...
        .oracle-inline-controls {
          display: grid;
          grid-template-columns: minmax(190px,.8fr) minmax(210px,.72fr) minmax(220px,.88fr);
          gap: 8px;
        }
        .oracle-inline-block {
          min-width: 0;
        ...
        .oracle-inline-seed-row::-webkit-scrollbar {
          display: none;
        }
        .oracle-inline-output-row .oracle-output-button {
          flex: 0 0 auto;
        }
        .oracle-inline-seed-row .oracle-seed-button {
        ...
        .oracle-followup-link:hover {
          ...
          background: rgba(217,70,239,0.055);
        }
