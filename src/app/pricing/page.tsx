...
const TIERS = [
  ...
  },
  {
    ...
    features: [
      ...
      { text: "Demo responses", included: true },
      { text: "Full correspondence web", included: false },
      { text: "Guided meditations", included: false },
      { text: "Unlimited questions", included: false },
    ],
...
export default function PricingPage() {
  ...
  useEffect(() => {
    fetch("/api/billing/session")
      .then(r => r.json())
      .then(d => {
        if (d.authenticated) setTier(d.plan || "guest");
      })
      .catch(() => {});
  }, []);

  async function handleSubscribe(tierId: string) {
    if (tierId !== "initiate") return;
  ...
  return (
    <div style={{ minHeight: "100vh", paddingTop: 100, background: "#0a090e", position: "relative" }}>
      {/* Background glow */}
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 1100, margin: "0 auto", padding: "0 24px 80px" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 64 }}>
          ...
          <h1 style={{ fontFamily: "'Cinzel Decorative', serif", fontSize: "clamp(28px, 5vw, 48px)", letterSpacing: "0.08em", background: "linear-gradient(135deg, #d946ef 0%, #d4a847 35%, #9333ea 65%, #22d3ee 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text", marginBottom: 16 }}>Access Tiers</h1>
          <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 18, color: "rgba(237,233,246,0.45)", maxWidth: 480, margin: "0 auto", lineHeight: 1.7 }}>
          ...
          {TIERS.map((t) => {
            ...
            return (
              ...
              >
                {/* Popular badge */}
                {t.popular && (
                  <div style={{ position: "absolute", top: -1, left: "50%", transform: "translateX(-50%)", padding: "4px 16px", border: "1px solid rgba(212,168,71,0.3)", borderTop: "none", background: "rgba(10,9,14,0.9)", fontFamily: "Cinzel, serif", fontSize: 8, letterSpacing: "0.2em", color: "#d4a847", textTransform: "uppercase", whiteSpace: "nowrap" }}>
                    Most Popular
                  </div>
                )}
                ...
                {isCurrentTier && !t.popular && (
                  <div style={{ position: "absolute", top: -1, left: "50%", transform: "translateX(-50%)", padding: "4px 14px", border: "1px solid rgba(255,255,255,0.1)", borderTop: "none", background: "rgba(10,9,14,0.9)", fontFamily: "Cinzel, serif", fontSize: 8, letterSpacing: "0.2em", color: "rgba(237,233,246,0.4)", textTransform: "uppercase", whiteSpace: "nowrap" }}>
                    Current Plan
                  </div>
                )}
                ...
                <div style={{ marginBottom: 24 }}>
                  ...
                  <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 14, color: "rgba(237,233,246,0.4)", lineHeight: 1.5 }}>{t.description}</p>
                </div>
                ...
                <ul style={{ listStyle: "none", padding: 0, margin: "0 0 32px", display: "flex", flexDirection: "column", gap: 10, flex: 1 }}>
                  {t.features.map((f) => (
                    <li key={f.text} style={{ display: "flex", alignItems: "center", gap: 10, fontFamily: "'Cormorant Garamond', serif", fontSize: 14, color: f.included ? "rgba(237,233,246,0.65)" : "rgba(237,233,246,0.2)" }}>
                      <Check size={13} color={f.included ? t.color : "rgba(107,101,128,0.3)"} strokeWidth={2.5} style={{ flexShrink: 0 }} />
                ...
                {t.id === "initiate" ? (
                  <button
                    ...
                    style={{
                      ...
                      background: isLoading ? "rgba(212,168,71,0.05)" : "rgba(212,168,71,0.06)",
                      color: isLoading ? "rgba(212,168,71,0.5)" : "#d4a847",
        ...
        <div style={{ maxWidth: 560, margin: "0 auto", textAlign: "center" }}>
          ...
          {[
            { q: "Can I cancel anytime?", a: "Yes — cancel from your member portal at any time. Your access continues until the end of the billing period." },
